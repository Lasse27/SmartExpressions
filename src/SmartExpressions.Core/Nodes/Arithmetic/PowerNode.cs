...
namespace SmartExpressions.Core.Nodes.Arithmetic
{
	public record PowerNode : ExpressionNode
	{
		...
		public override Operation<object> Evaluate(Evaluator evaluator)
		{
			...
			Operation<decimal> resolvedLeft = EvaluatorHelpers.ResolveDecimal(rawLeft, "Pow.1");
			...
			Operation<decimal> resolvedRight = EvaluatorHelpers.ResolveDecimal(rawRight, "Pow.2");
			if (resolvedRight.Status == Status.Failure) { return Operation<object>.Failure(resolvedRight.Message); }

			// Mult adn return
			return Operation<object>.Success(Math.Pow((double)resolvedLeft.Value, (double)resolvedRight.Value));
		}
	}
}
