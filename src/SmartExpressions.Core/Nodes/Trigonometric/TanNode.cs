...
{
	public record TanNode : ExpressionNode
	{
		...
		/// <inheritdoc/>
		public TanNode(ExpressionNode operand) => this.Operand = operand;
		...
		public static NodeResult Get(Parser parser)
		{
			...
			ExpressionNode node = new TanNode(operand.GetValue());
		...
		public override EvaluationResult Evaluate(EvaluationContext ctx)
		{
			...
			double absolute = Math.Tan(resolved.Value);
