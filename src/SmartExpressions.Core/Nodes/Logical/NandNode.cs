...
namespace SmartExpressions.Core.Nodes.Logical
{
	...
	public record NandNode : BinaryFunction
	{
		private const string Keyword = "NAND";

		/// <inheritdoc/>
		public NandNode(ExpressionNode left, ExpressionNode right) : base(left, right) { }
		...
		public static NodeResult Get(Parser parser)
		{
			...
			ExpressionNode node = new NandNode(dualOperand.Value.Left, dualOperand.Value.Right);
		...
		public override EvaluationResult Evaluate(EvaluationContext ctx)
		{
			...
			// Short circuit
			if (resolvedLeft.Value == false && ctx.Settings.ShortCircuitExpressions)
			{
